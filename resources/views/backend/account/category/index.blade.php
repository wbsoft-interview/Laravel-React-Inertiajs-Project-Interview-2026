...
@section('title') Account Category | Master Template @endsection
@section('account-category') active @endsection
@section('account-category.index') active @endsection
@section('styles')
@endsection
...
<div class="row py-3 ps-2">
    <div class="heading d-flex justify-content-start align-items-center">
        <h3 class="mb-0"><span> Account Category</span></h3>
    </div>
</div>
...
<div class="table_wrapper py-1 card">
        ...
        <div class="px-3 ">
            <div class=" d-flex justify-content-between align-items-center aggregate-section border">
                <div class="d-flex align-items-center">
                    <p class="mb-0"><a href="{{ route('account-category.index') }}"
                            class="text-primary py-2 px-3 active">All({{$allAccountCategoryCount}})</a>
                    </p>
                </div>
                <div class="d-sm-block">
                    @if(Auth::user()->can('account-category-create'))
    ...
    <div class="row px-3">
        <div class="table-container table-responsive">
            <table id="" class="table table-bordered">
                ...
                <tbody class="text-center">
                    ...
                    @foreach($accountCategoryData as $key=>$item)
                        ...
                        <td class="text-center">
                            ...
                            <div class="row-actions mt-2">
                                ...
                                <span> | <a class="text-warning fw-bolder"
                                        href="{{route('account-category-inactive', $item->id)}}">Inactive</a></span>
                                @else
                                <span> | <a class="text-success fw-bolder"
                                        href="{{route('account-category-active', $item->id)}}">Active</a>
                                </span>
                                @endif
                                ...
                                <span> | <a class="text-danger fw-bolder row-delete"
                                        href="{{route('account-category-delete', $item->id)}}">Delete</a>
                                </span>
                                @endif
                            </div>
                        </td>
                    ...
                    <div class="modal fade" id="updateAccount{{$item->id}}" tabindex="-1"
                        ...
                        <div class="modal-dialog modal-dialog-centered max-width-900px">
                            <div class="modal-content">
                                ...
                                <form action="{{route('account-category.update', $item->id)}}" method="post"
                                        ...
                                        <div class="row px-4 my-4">
                                            <div class="col-md-12 mb-3">
                                                <div class="form-group">
                                                    <label for="account_name">Account Category Name<span
                                                            class="text-danger">*</span>
                                                    </label>
                                                    <input type="text" name="category_name" required
                                                        class="form-control form-control-solid"
                                                        value="{{$item->category_name}}" placeholder="Account category Name" required>
                                                </div>
        ...
        @if(isset($allAccountCategoryCount) && $allAccountCategoryCount > 10)
        <div class="clearfix d-flex">
            <div class="float-left">
                <p class="text-muted">
                    {{ __('Showing') }}
                    <span class="font-weight-bold">{{ $accountCategoryData->firstItem() }}</span>
                    {{ __('to') }}
                    <span class="font-weight-bold">{{ $accountCategoryData->lastItem() }}</span>
                    {{ __('of') }}
                    <span class="font-weight-bold">{{ $accountCategoryData->total() }}</span>
                    {{ __('results') }}
                </p>
            </div>

            <div class="float-right custom-table-pagination">
                {!! $accountCategoryData->links('pagination::bootstrap-4') !!}
            </div>
        </div>
        @endif
    </div>
            ...
            <form action="{{ route('account-category.store') }}" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body p-0">
                    <div class="row px-4 my-4">
                        <div class="col-md-12 mb-3">
                            <div class="form-group">
                                <label for="account_name">Account Category Name<span class="text-danger">*</span>
                                </label>

...
<div class="table_wrapper py-1 card">
        ...
        <div class="px-3 ">
            <div class=" d-flex justify-content-between align-items-center aggregate-section border">
                <div class="d-flex align-items-center">
                    <p class="mb-0"><a href="{{route('expense-category.index')}}"
                            class="text-primary py-2 px-3 active">All({{$allExpenseCategoryCount}})</a>
                    </p>
                </div>
                <div class="d-sm-block">
                    @if(Auth::user()->can('expense-category-create'))
    ...
    <div class="row px-3">
        <div class="table-container table-responsive">
            <table id="" class="table table-bordered">
                ...
                <tbody class="text-center">
                    ...
                    @foreach($expenseCategoryData as $key=>$item)
                        ...
                        <td>
                            ...
                            <div class="row-actions mt-2">
                                ...
                                <span><button class="text-primary border-0 bg-transparent fw-bolder" value="{{ $item->id }}"
                                        onclick="updateExpenseCategory({{$item->id}})"> Edit </button></span>
                                

                                @if($item->status == true)
                                <span> | <a class="text-warning fw-bolder"
                                        href="{{route('expense-category-inactive', $item->id)}}">Inactive</a></span>
                                @else
                                <span> | <a class="text-success fw-bolder"
                                        href="{{route('expense-category-active', $item->id)}}">Active</a>
                                </span>
                                @endif
                                ...
                                <span> | <a class="text-danger fw-bolder row-delete"
                                        href="{{route('expense-category-delete', $item->id)}}">Delete</a>
                                </span>
                                @endif
                            </div>
                        </td>
                    ...
                    <div class="modal fade" id="updateExpenseCategory{{$item->id}}" tabindex="-1"
                        ...
                        <div class="modal-dialog modal-dialog-centered max-width-900px">
                            <div class="modal-content">
                                ...
                                <form action="{{route('expense-category.update', $item->id)}}" method="post"
        ...
        @if(isset($allExpenseCategoryCount) && $allExpenseCategoryCount > 10)
        <div class="clearfix d-flex">
            <div class="float-left">
                <p class="text-muted">
                    {{ __('Showing') }}
                    <span class="font-weight-bold">{{ $expenseCategoryData->firstItem() }}</span>
                    {{ __('to') }}
                    <span class="font-weight-bold">{{ $expenseCategoryData->lastItem() }}</span>
                    {{ __('of') }}
                    <span class="font-weight-bold">{{ $expenseCategoryData->total() }}</span>
                    {{ __('results') }}
                </p>
            </div>

            <div class="float-right custom-table-pagination">
                {!! $expenseCategoryData->links('pagination::bootstrap-4') !!}
            </div>
        </div>
        @endif
    </div>
...
<div class="modal fade" id="addExpenseCategory" tabindex="-1" aria-labelledby="oneInputModalLabel" aria-hidden="true" data-bs-backdrop='static'>
    <div class="modal-dialog modal-dialog-centered max-width-900px">
        <div class="modal-content">
            <div class="modal-header">
...
<script>
    ...
    function updateExpenseCategory(id) {
        $("#updateExpenseCategory"+id).modal('show');
    }
</script>
@endsection
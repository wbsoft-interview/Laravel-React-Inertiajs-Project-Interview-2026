...
@section('title') Division | Beauty Parlour @endsection
@section('division-list') active @endsection
@section('styles')
@endsection
...
<div class="row py-3 ps-2">
    <div class="heading d-flex justify-content-start align-items-center">
        <h3 class="mb-0"><span> Division List</span></h3>
    </div>
</div>
...
<div class="table_wrapper py-1 card">
        ...
        <div class="px-3 ">
            <div class=" d-flex justify-content-between align-items-center aggregate-section border">
                <div class="d-flex align-items-center">
                    <p class="mb-0"><a href="{{route('division-list')}}"
                            class="text-primary py-2 px-3 active">All({{$allDivisionCount}})</a>
                    </p>
                </div>
                <div class="d-sm-block">
                    ...
                    <a href="javascript:void(0)"
                        ...
                        <span class="">Division List</span></a>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="row px-3">
        <div class="table-container table-responsive">
            <table id="" class="table table-bordered">
                ...
                <tbody class="text-center">
                    ...
                    @foreach($divisionData as $key=>$item)
                    ...
                    <tr class="">
                        ...
                        <td class="text-start">
                            ...
                            <div class="row-actions mt-2">
                                ...
                                <span><button class="text-primary border-0 bg-transparent fw-bolder"
                                        value="{{ $item->id }}" onclick="updateZone({{$item->id}})"> Edit
                                    </button></span>
                                @endif
        ...
        @if(isset($allDivisionCount) && $allDivisionCount > 10)
        <div class="clearfix d-flex">
            <div class="float-left">
                <p class="text-muted">
                    {{ __('Showing') }}
                    <span class="font-weight-bold">{{ $divisionData->firstItem() }}</span>
                    {{ __('to') }}
                    <span class="font-weight-bold">{{ $divisionData->lastItem() }}</span>
                    {{ __('of') }}
                    <span class="font-weight-bold">{{ $divisionData->total() }}</span>
                    {{ __('results') }}
                </p>
            </div>

            <div class="float-right custom-table-pagination">
                {!! $divisionData->links('pagination::bootstrap-4') !!}
            </div>
        </div>
        @endif
    </div>

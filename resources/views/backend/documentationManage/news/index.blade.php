...
@section('title') Documentation | Sangbad Protikhon @endsection
@section('documentation') active @endsection
@section('documentation.index') active @endsection
@section('styles')
@endsection
...
<div class="row py-3 ps-2">
    <div class="heading d-flex justify-content-start align-items-center">
        <h3 class="mb-0"><span> Documentation List</span></h3>
    </div>
</div>
                            ...
                            <div class="row-actions">
                                @if (Auth::user()->can('documentation-edit'))
                                    <span><button class="edit_class_modal border-0 bg-transparent fw-bolder"> 
                                        <a class="text-primary" href="javascript:void(0)" onclick="updateNewsTag({{ $item->id }})">View</a> </button> | </span>
                                        
                                    <span><button class="edit_class_modal border-0 bg-transparent fw-bolder"
                                        value="{{ $item->id }}"> <a class="text-info" href="{{route('documentation.edit', $item->id)}}">Edit</a> </button> | </span>
                                @endif

                                @if (Auth::user()->can('documentation-delete'))
                                <span>  <a class="text-danger fw-bolder row-delete" href="{{route('documentation-delete', $item->id)}}">Delete</a>
                                </span>
                                @endif
                            </div>
                        </td>
